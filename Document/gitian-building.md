...
*Setup instructions for a Gitian build of Wdgc Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Wdgc
...
- [Building Wdgc Core](#building-wdgc-core)
...
Clone the git repositories for Wdgc Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/wdgcpay/wdgc
...
in the Wdgc Core repository under 'Fetch and create inputs' to install sources which require
...
To build Wdgc Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Wdgc Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/wdgc/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/wdgcpay/wdgc
...
For example:
```bash
URL=https://github.com/crowning-/wdgc.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit wdgc=${COMMIT} --url wdgc=${URL} ../wdgc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit wdgc=${COMMIT} --url wdgc=${URL} ../wdgc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit wdgc=${COMMIT} --url wdgc=${URL} ../wdgc/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the wdgc git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../wdgc/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/wdgc
SIGPATH=/some/root/path/wdgc-detached-sigs

./bin/gbuild --url wdgc=${BTCPATH},signature=${SIGPATH} ../wdgc/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/wdgc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/wdgc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/wdgc-osx-build.assert
```
...
[wdgc/gitian.sigs](https://github.com/wdgcpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
